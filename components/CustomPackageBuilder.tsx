...
export default function CustomPackageBuilder() {
    ...
    const quoteMessage = useMemo(() => {
        ...
        const monthlyPrice = paymentPlan === "3months"
            ? Math.ceil((totalPrice * 1.10) / 3)
            : Math.ceil((totalPrice * 1.15) / 6);
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        >
                                            CHF {paymentPlan === "onetime"
                                                ...
                                                : paymentPlan === "3months"
                                                    ? Math.ceil((totalPrice * 1.10) / 3).toLocaleString()
                                                    : Math.ceil((totalPrice * 1.15) / 6).toLocaleString()
                                            }
                                    ...
                                    {paymentPlan !== "onetime" && (
                                        <p className="text-xs text-slate-500 mb-4">
                                            Total: CHF {paymentPlan === "3months"
                                                ? Math.ceil(totalPrice * 1.10).toLocaleString()
                                                : Math.ceil(totalPrice * 1.15).toLocaleString()
                                            } over {paymentPlan === "3months" ? "3" : "6"} months
                                            <span className="text-amber-400/80"> (+{paymentPlan === "3months" ? "10" : "15"}%)</span>
                                        </p>
                                    )}
